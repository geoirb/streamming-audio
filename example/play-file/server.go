...
type configuration struct {
	...
	PlayFile string `envconfig:"FILE" default:"example/play-file/server/test.wav"`
}
...
func main() {
	...
	pwd, _ := os.Getwd()
	file := fmt.Sprintf("%s/%s", pwd, cfg.PlayFile)
	playerIP := "127.0.0.1"
	playerPort := "8083"
	playerDevice := "hw:0,0"
	uuid, _, _, _ := svc.PlayAudioFile(context.Background(), playerIP, playerPort, file, playerDevice)

	c := make(chan os.Signal, 1)
	signal.Notify(c, syscall.SIGTERM, syscall.SIGINT)
	level.Error(logger).Log("msg", "received signal, exiting signal", "signal", <-c)

	svc.Stop(context.Background(), playerIP, playerPort, playerDevice, uuid)
}
